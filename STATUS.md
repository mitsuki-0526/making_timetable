...
### 特別支援学級の自動連動
- 通常クラスに教科をセットすると、`settings.mappingRules` に基づいて同学年の特支クラスへ自動で対応教科をセット
- ルールが設定されていない教科は連動しない
...
タブ構成：
1. **クラス設定** — 学年・クラス・特支クラスの追加・削除
2. **教科設定** — 教科の追加・削除・規定時数・連続日数制約・特支連動ルール
