...
function tryOnce({
  ...
  const slotOk = (grade, className, subject, day, period) => {
    ...
    // 許可曜日チェック
    if (sp?.allowed_days?.length > 0 && !sp.allowed_days.includes(day))
      return false;

    // 許可時限チェック
    if (sp?.allowed_periods?.length > 0 && !sp.allowed_periods.includes(period))
      return false;
      ...
      for (const { day, period } of slots) {
        ...
        if (!assignment) {
          for (const g of teacherGroups || []) {
            ...
            if (gg.length > 0 && ![...gradeSet].some((gr) => gg.includes(gr)))
              continue;
            if (teacherUsage.has(`${g.id}|${day}|${period}`)) continue;
            assignment = {
              teacher_id: null,
