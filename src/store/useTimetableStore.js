...
export const useTimetableStore = create((set, get) => ({
    ...
    set((state) => {
      ...
      if (teacher_id || subject) {
        currentTimetable.push({
          ...
          teacher_id,
          subject,
          ...(preservedCellGroupId
            ? { cell_group_id: preservedCellGroupId }
            : {}),
        });
      }
      ...
      const upsertSubject = (targetClass, targetSubject) => {
        ...
        // 既存教員がそのまま使えるか確認
        const prevTeacherId =
          idx >= 0 ? currentTimetable[idx].teacher_id : null;
        ...
        const prevTeacherIsTokkiShien =
          prevTeacher?.subjects.includes("特別支援");
        let newTeacherId =
        ...
        if (!newTeacherId) {
          const suitable = state.teachers.find((t) => {
            ...
            const alreadyUsed = currentTimetable.some((e) => {
              if (e.day_of_week !== day_of_week || e.period !== period)
                return false;
  ...
  removeSubjectPairing: (id) => {
    set((state) => ({
      subject_pairings: (state.subject_pairings || []).filter(
        (p) => p.id !== id,
      ),
    }));
  },
  ...
  removeCrossGradeGroup: (id) => {
    set((state) => ({
      cross_grade_groups: (state.cross_grade_groups || []).filter(
        (g) => g.id !== id,
      ),
    }));
  },
  ...
  removeSubject: (subjectToDelete) => {
    set((state) => {
      ...
      const newSettings = {
        ...state.settings,
        mappingRules: { ...state.settings.mappingRules },
      };
  ...
  getAvailableTeachers: (
    ...
    return state.teachers.filter((teacher) => {
      ...
      if (
        !isTokkiShien &&
        typeof target_grade !== "string" &&
        !teacher.target_grades.includes(target_grade)
      ) {
        return false;
      }
  ...
  removeFacility: (id) => {
    set((state) => ({
      ...
      subject_facility: Object.fromEntries(
        Object.entries(state.subject_facility || {}).filter(
          ([, v]) => v !== id,
        ),
      ),
    }));
  ...
  removeSubjectSequence: (id) => {
    set((state) => ({
      subject_sequences: (state.subject_sequences || []).filter(
        (s) => s.id !== id,
      ),
    }));
  },
  ...
  importState: (newState) => {
    set({
      ...
      subject_constraints:
        newState.subject_constraints || dummySubjectConstraints,
