...
このモーダル（ポップアップ窓）には、基礎構成を管理する複数のタブがあります。

- 上端の項目タブ帯は、入りきらないときだけ左右に移動できる作りです。上下には動かないようにしてあり、画面が少しだけずれることを防いでいます。
- タブの中身は縦にスクロールできます。項目が多い学校でも、モーダルの高さを超えた分を下へたどって編集できます。

### タブ①：授業時限数

- **何をするか**: 月曜は5限まで、木曜は6限まで、といった曜日ごとの最大時限数を設定します
- **使い道**: ここで決めた上限をもとに、時間割表の表示・編集・Excel/PDF 出力・自動生成の対象となるコマ数が決まります
- **補足**: 使わない時限は時間割表の中で無効セルとして表示されます
